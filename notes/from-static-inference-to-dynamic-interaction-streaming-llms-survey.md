...
论文把模型过程抽象为条件分布 \(P(Y|X)\)，其中输入流为 \(X=(x_1,...,x_M)\)，输出流为 \(Y=(y_1,...,y_N)\)。关键是引入一个决策函数 \(\phi(t)\)：在生成第 \(t\) 个输出 token 时，模型到底能看到输入流的哪一部分。
...
这是 \(X \rightarrow Y\) 形式，模型主要做实时感知和输出。
...
这是 \(X \rightarrow Z \rightarrow Y\) 形式，模型不只是直接映射，还要维护 latent state，用于推理、规划或工具调用。
